...
def optimize_logits_only(desired_ending_ids, prefix_length, batch_size):
    '''
    ...
    if True:
        optimized_word_logits = torch.nn.Parameter(
            torch.rand([batch_size, prefix_length, tokenizer.vocab_size], device=device)
        )
    else:
    ...
    for iter in range(2000):
        ...
        if False:
            ...
            optimized_word_probs_no_temp = optimized_word_logits
        ...
        # optimized_word_probs = optimized_word_logits
        optimized_embeddings = torch.matmul(optimized_word_probs, model.get_input_embeddings().weight)
...
def experiment1():
    ...
    optimize_logits_only(desired_ending_ids, prefix_length, batch_size)
